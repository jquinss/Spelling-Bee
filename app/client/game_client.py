        ...
        if join_response_code == 0:
            ...
            while self.game_loop:
                ...
                if word == "\\s":
                    ...
                    response = self.stub.SubmitWord(WordSubmissionRequest(gameId=game_id,
                                                                          username=username,
                                                                          word=word.lower()))
...
class CreateGameMenu(Menu):
    ...
    def create_singleplayer_game(self):
        ...
        username = self._create_username()
        game_id = self.game_stub.CreateGame(CreateGameRequest(username=username,
                                                              gameType=self.game_type, gameMode=game_mode)).gameId
        self.game_stub.InitGame(InitGameRequest(gameId=game_id))
        letters = self.game_stub.GetPangram(GetPangramRequest(gameId=game_id))
        self.game_loop = True
        while self.game_loop:
            print(letters)
            word = input("Enter a word (or command - enter \\h for a list of available commands): ")
            if word == "\\s":
                status = self.game_stub.QueryGameStatus(GameStatusRequest(gameId=game_id))
                # TO DO - print status
            else:
                response = self.game_stub.SubmitWord(WordSubmissionRequest(gameId=game_id,
    ...
    def create_multiplayer_coop_game(self):
        ...
        print("Requesting username")
        username = self._create_username()
        create_game_response = self.game_stub.CreateGame(CreateGameRequest(username=username,
                                                                            gameType=self.game_type,
                                                                            gameMode=game_mode))
        game_id = create_game_response.gameId
        join_code = create_game_response.joinCode
        response_code = self.game_stub.InitGame(InitGameRequest(gameId=game_id)).responseCode
